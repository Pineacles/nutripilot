"""Agent-facing OpenAPI description for the FastAPI app.

Moved out of app/main.py to keep that module focused on app wiring.
"""
...
| **Supplements** | Define supplement profiles with dose and micronutrient content. Log daily supplement intake with dose, unit, and time of day. List a day's supplement logs. Update or delete entries |
| **Water** | Log water intake (ml). List a day's water logs. Update or delete entries |
| **Caffeine** | Tracked automatically via food nutrients (`caffeine_mg` per 100g). Log a caffeinated food/drink and caffeine is counted. No separate caffeine logging needed. Legacy caffeine logs (if any) can still be listed |
